...
jsPsych.plugins["categorize-animation-rsvp"] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('categorize-animation-rsvp', 'stimuli', 'image');

  plugin.info = {
    name: 'categorize-animation-rsvp',
    description: '',
    parameters: {
      stimuli: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var responded = false;
    var ready_for_feedback = true;
    var feedback_displayed = false;
    ...
    var animate_interval = setInterval(function() {
      ...
      if (responded & ready_for_feedback){
        display_element.innerHTML += '<div class="boxedDP">COLOURED<br>PICTURE HERE</div>';
        // display_element.innerHTML += '<p>testtesttest</p>';
        // "<p style='color:#00FF00;font-size:180%;line-height:150%;'>COLOURED<br>PICTURE HERE</p>";
        ready_for_feedback = false
      } else if (responded & !feedback_displayed){
        // set timeout to clear feedback
        sleep(trial.feedback_duration);
      ...
      if (showAnimation & trial.allow_response_before_complete) { 
    ...
    var after_response = function(info) {
      ...
      trial_data = {
        ...
        "correct": correct,
        "key_press": info.key,
        "stream_frame": animate_frame // this is the frame that the stream was up to when the participant pressed the button
        // remember that this is 0-indexed (so need to add 100 ms if converting into a time)
      };
